...
const ICONS = {
  ...
  MoreHorizontal: ({s,c}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>,
};
...
function AppShell() {
  ...
  // Footer structure: 5 items max
  // Index 1: Home, Index 2: Pinned[0], Index 3: Invoices (CENTER), Index 4: Pinned[1], Index 5: More/Settings
  ...
  const navItems = [
    ...
    pinnedModules[0] ? { id: pinnedModules[0].route, icon: pinnedModules[0].icon, label: rtl ? pinnedModules[0].nameAr : pinnedModules[0].name, color: pinnedModules[0].color } : { id: "expenses", icon: "Wallet", label: t("expenses"), color: "#6366f1" },
    { id: "invoices", icon: "Receipt", label: t("invoices"), color: "#f43f5e", isCenter: true },
    pinnedModules[1] ? { id: pinnedModules[1].route, icon: pinnedModules[1].icon, label: rtl ? pinnedModules[1].nameAr : pinnedModules[1].name, color: pinnedModules[1].color } : { id: "groceries", icon: "ShoppingCart", label: t("groceries"), color: "#10b981" },
    { id: "more", icon: "MoreHorizontal", label: t("more"), isMore: true },
  ];
  ...
  return (
    <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <button onClick={() => { setPage("settings"); setMoreOpen(false); }}
                ...
                <div style={{
                  width: 40, height: 40, borderRadius: 12,
                  background: "linear-gradient(135deg, rgba(160,160,176,0.15), rgba(160,160,176,0.05))",
                  border: "1px solid rgba(160,160,176,0.15)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}>
                  <ModuleIcon name="Settings" size={20} color="#a0a0b0" />
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{t("settings")}</div>
                  <div style={{ fontSize: 11, color: "#606070", marginTop: 2 }}>App config</div>
                </div>
              </button>
            </div>
          </div>
      ...
      {/* Bottom Navigation - 5 items, Invoices at index 3 (center) */}
      <nav style={{
          ...
          {navItems.map((item, idx) => {
            ...
            return (
              <button key={item.id} onClick={() => {
                ...
                width: isCenter ? 64 : 56, height: isCenter ? 64 : 52,
                ...
                <ModuleIcon 
                  ...
                  color={isCenter ? "#fff" : active ? (item.color || "#f0f0f5") : "#505060"} 
                />
                {!isCenter && <span style={{ fontSize: 10, whiteSpace: "nowrap" }}>{item.label}</span>}
                {active && !isCenter && !isMore && (
                  <span style={{
